...
// import { state, memo, batch, Getter, Setter, peek, Accessor } from "../core/signals.js";
// import { createMarkup } from "../core/markup/utils.js";
// import { Context, onEffect } from "../core/context.js";
...
// export function createVirtualList<T>(this: Context, props: VirtualListProps<T>): [() => any, VirtualListAPI<T>] {
...
//       if (!options.smooth) scrollTop(targetOffset);
//     },
...
//     const renderedNode = props.render(item as Getter<T>, index, { isEntering });
...
//     const currentAvg = peek(averageHeight);
...
//         slot.index(i);
//         slot.offset(getOffset(i, currentAvg));
//         slot.active(true);
...
//     let currentCount = measuredCount();
//     let currentTotal = totalMeasuredHeight();
...
//       const effectivelyAtBottom = isAtBottom() || isAutoGliding;
...
//   let previousCount = props.items().length;
//   let previousFirstKey: string | number | null = props.items()[0] ? props.keyFn(props.items()[0]) : null;
...
//       const addedHeight = prependedCount * peek(averageHeight);
//       if (viewportElement) {
...
//         props.renderEmpty && props.items().length === 0 ? props.renderEmpty() : null,
