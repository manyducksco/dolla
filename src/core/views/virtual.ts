...
export function createVirtualList<T>(props: VirtualListProps<T>): [() => any, VirtualListAPI<T>] {
  ...
  const api: VirtualListAPI<T> = {
    ...
    scrollToItem: (item: T, options = {}) => {
      const index = props.items.get().indexOf(item);
  ...
  function updateScrollSlice() {
    ...
    if (props.isSticky && props.renderSticky) {
      const indices = stickyIndices.get();
      let activeIdx = -1;
      let nextIdx = -1;
      ...
      if (activeStickyItem.get() !== newActive) activeStickyItem.set(newActive);
      ...
      if (stickyPushOffset.get() !== push) stickyPushOffset.set(push);
    }
  ...
  const observer = new ResizeObserver((entries) => {
    ...
    if (needsRecalculation) {
      ...
      const effectivelyAtBottom = isAtBottom.get() || isAutoGliding;
  ...
  let previousFirstKey: string | number | null = props.items.get()[0] ? props.keyFn(props.items.get()[0]) : null;
